...
object DataSkew02 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawRDD = sc.textFile("D:\\data_skew.txt")//读取数据源
//        val rawRDD=args(0)

    val reducedRDD_01 = rawRDD.map(line => {/**解决倾斜第一步：加盐操作将原本1个分区的数据扩大到100个分区*/
      ...
      if (target_ip.equals("106.38.176.185")){/**针对特定倾斜的key进行加盐操作*/
        ...
        (target_ip + "-" + salt,List(subClientIP))
      }
      else (target_ip,List(subClientIP))
    }).groupByKey(103)
