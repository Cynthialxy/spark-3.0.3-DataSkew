...
private[scheduler] class BlacklistTracker (
  ...
  def applyBlacklistTimeout(): Unit = {
    ...
    // quickly check if we've got anything to expire（到期） from blacklist -- if not, avoid doing any work
    if (now > nextExpiryTime) {  //如果现在已经超过下一次到期的时间
      // Apply the timeout to blacklisted nodes and executors
      val execsToUnblacklist = executorIdToBlacklistStatus.filter(_._2.expiryTime < now).keys  //过滤出已到期的executor
      if (execsToUnblacklist.nonEmpty) {  //如果存在已到期的executor
        ...
        execsToUnblacklist.foreach { exec =>
          ...
          if (failedExecsOnNode.isEmpty) {  //如果该节点的黑名单为空
            nodeToBlacklistedExecs.remove(status.node)  //从黑名单中移除该节点
          }
        }
      }
      val nodesToUnblacklist = nodeIdToBlacklistExpiryTime.filter(_._2 < now).keys  //过滤出已到期的node
      if (nodesToUnblacklist.nonEmpty) {  //如果存在已到期的node
        ...
        nodesToUnblacklist.foreach { node =>
          nodeIdToBlacklistExpiryTime.remove(node)  //移除已到期的node
