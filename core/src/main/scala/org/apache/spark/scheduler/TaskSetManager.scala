...
private[spark] class TaskSetManager(
  ...
  private def getLocalityWait(level: TaskLocality.TaskLocality): Long = {
    val localityWait = level match {
      case TaskLocality.PROCESS_LOCAL => config.LOCALITY_WAIT_PROCESS  //默认3秒
      case TaskLocality.NODE_LOCAL => config.LOCALITY_WAIT_NODE  //默认3秒
      case TaskLocality.RACK_LOCAL => config.LOCALITY_WAIT_RACK  //默认3秒
      case _ => null  //不做等待
    }

    if (localityWait != null) {   //如果TaskLocality的level不是NO_PREF或者ANY，等待一段时间
  ...
  private def computeValidLocalityLevels(): Array[TaskLocality.TaskLocality] = {
    ...
    //将等待期间的task依次进行分配
    if (!pendingTasks.forExecutor.isEmpty &&  //如果task对应的executor存在且有可用资源，将PROCESS_LOCAL加入本地优先级levels
        pendingTasks.forExecutor.keySet.exists(sched.isExecutorAlive(_))) {
      levels += PROCESS_LOCAL
    }
    if (!pendingTasks.forHost.isEmpty &&  //如果task对应的host存在且有可用资源，将NODE_LOCAL加入本地优先级levels
        pendingTasks.forHost.keySet.exists(sched.hasExecutorsAliveOnHost(_))) {
      levels += NODE_LOCAL
    }
    if (!pendingTasks.noPrefs.isEmpty) {  //如果存在本地优先级为noPrefs的任务，将NO_PREF加入levels
      levels += NO_PREF
    }
    if (!pendingTasks.forRack.isEmpty &&   //如果task对应的rack存在且有可用资源，将RACK_LOCAL加入本地优先级levels
        pendingTasks.forRack.keySet.exists(sched.hasHostAliveOnRack(_))) {
      levels += RACK_LOCAL
    }
    levels += ANY   //直接将ANY加入本地优先级levels
