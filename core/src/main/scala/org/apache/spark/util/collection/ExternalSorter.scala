...
private[spark] class ExternalSorter[K, V, C](
  ...
  //noinspection ScalaStyle
  //数据在内存中并不进行排序，而是按照是否预聚合，写入map或者buffer
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    // TODO: stop combining if we find that the reduction factor isn't high
    val shouldCombine = aggregator.isDefined
    if (shouldCombine) {
      //如果map端需要预聚合，更新value的值
      ...
      while (records.hasNext) {
        addElementsRead()
        kv = records.next()
        map.changeValue((getPartition(kv._1), kv._1), update)   //将聚合后的value和对应的key写入map
        maybeSpillCollection(usingMap = true)  //判断是否需要溢写
      }
    } else {
      //如果不需要预聚合，直接插入buffer中
      // Stick values into our buffer
      while (records.hasNext) {
        addElementsRead()
        val kv = records.next()
        buffer.insert(getPartition(kv._1), kv._1, kv._2.asInstanceOf[C])   //将key和value直接写入Buffer
        maybeSpillCollection(usingMap = false)  //判断是否需要溢写
      }
    }
  }
  ...
  private def maybeSpillCollection(usingMap: Boolean): Unit = {
    ...
    if (usingMap) {
      //如果需要预聚合，用map存储
      estimatedSize = map.estimateSize()   //估计数据量大小
      if (maybeSpill(map, estimatedSize)) {
        map = new PartitionedAppendOnlyMap[K, C]
      }
    } else {
      //如果不需要预聚合，用buffer存储
      estimatedSize = buffer.estimateSize()   //估计数据量大小
      if (maybeSpill(buffer, estimatedSize)) {
        buffer = new PartitionedPairBuffer[K, C]
      }
    }

    if (estimatedSize > _peakMemoryUsedBytes) { //如果数据量大小大于内存峰值
      _peakMemoryUsedBytes = estimatedSize   //将内存峰值调整为数据量大小
    }
  }
   ...
   */
    //溢写到文件中时，需要按照指定的排序规则进行排序(comparator)
